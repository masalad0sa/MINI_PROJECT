...
class BehaviorAnalyzer:
    ...
    def analyze(self, gaze, head):
        ...
        if abnormal:
            ...
            # Require sustained abnormal behavior (3+ consecutive frames)
            # before raising the score. This prevents momentary glances
            # from triggering false violations.
            can_increment = self.abnormal_streak >= 3
            if can_increment:
                increment = 0.5
                if gaze_abnormal and head_abnormal:
                    increment += 0.5
                if self.abnormal_streak >= 6:
                    increment += 0.5
            ...
            # Fast decay so score drops quickly when student looks back
            decay_rate = 6.0 if self.normal_streak >= 2 else 4.0
